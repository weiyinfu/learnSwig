 ...
 */
 void SetCustomUserRole(IRtcEngine* engine, const char* role);

/** 
 * @brief 设置 ByteRTC SDK 默认开启使用QUIC获取配置
 * @notes  一般在linux端SDK使用
 */
 void SetDefaultEnableQuicGetAccess();

/** 
 * @brief 设置 ByteRTC SDK 默认开启使用KCP获取配置
 * @notes  一般在linux端SDK使用
 */
 void SetDefaultEnableKcpGetAccess();

/** 
 * @brief 设置 ByteRTC SDK 默认开启通话前网络探测
 * @notes  一般在linux端SDK使用
 */
 void SetDefaultEnableNetworkDetect();

/** 
 * @brief 设置 ByteRTC SDK 默认开启使用KCP消息通道
 * @notes  一般在linux端SDK使用
 */
 void SetDefaultEnableKcpDataChannel();
 ...
 */
 void SetEngineInternalEventHandler(
      IRtcEngine *engine,
