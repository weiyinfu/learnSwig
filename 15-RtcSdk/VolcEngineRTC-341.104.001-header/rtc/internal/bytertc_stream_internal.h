 ...
 */
 int StartPushStream(IRtcEngine* engine, const char* token, const char* stream_id);

/** 
 * @brief ByteRTC SDK 停止推无房间流
 * @notes  一般在linux端SDK使用
 */
 void StopPushStream(IRtcEngine* engine);
 ...
 */
 void SetEngineInternalStreamEventHandler(IRtcEngine *engine, IEngineInternalStreamEventHandler* handler);
